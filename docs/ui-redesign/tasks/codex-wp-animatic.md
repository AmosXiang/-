...
- **允许新增的文件仅限**：§二两个源文件、其测试、§四的验收证据文档（`evidence/animatic-acceptance.md` 及其截图）——此外零新增；**禁碰 App.tsx、server.ts、router.ts、main.tsx、types.ts、任何 server/modules/**——主工作区当前有已知未提交改动（用户本人的进行中工作，涉及 App.tsx/server.ts 等），你从干净基线拉的 worktree 看不到它们，碰了必然合并冲突；挂载接线全部由 CC 做；

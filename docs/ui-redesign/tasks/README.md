...
| 工作包 | 分支 | 基线 |
| --- | --- | --- |
| WP-A (CC) | `fix/p0-hotfix` | 待定基线（见下） |
| WP-B (Codex) | `feat/shot-review-api` | 同一基线 |
| WP-C (Antigravity) | `feat/export-deck-api` | 同一基线 |

**前置条件（阻塞并行开工）**：当前工作区在 `feature/camera-derive` 上有 9 个未提交改动文件（含 `server.ts`、`src/App.tsx`、`src/types.ts`）。必须先把这些改动落库（或明确丢弃），确定统一基线 commit，三路分支再从该基线拉出。否则三个 agent 会围绕同一批未落库改动产生冲突。

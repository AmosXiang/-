...
## CC 复核与真机确认（2026-07-17）

逐行 review PASS：
- server.ts 仅 `registerImageGenRouting` 调用点单 hunk，用既有 import 的 `resolveEffectiveStyleContract` + `sceneForShot` + `readDb` 组装，try/catch 返 null——热区零越界。
- 框架语两条前缀与 server.ts ComfyUI 路径（L7216/L7226）**逐字一致**（已字面比对）；`composeAgnesPrompt` 的 includes 去重、优化文→项目→场景顺序与 ComfyUI 对齐。
- 尺寸继承用 `=== undefined` 判定（显式 width=0 不塌陷回 1024）；版本成功写、bundle 缺席清（`=== null` → delete）；`rawMeta8k` 升级为字节感知截断且保留 styleBundle 摘要+合法 JSON。
- imageGen 模块无直接 import style-contract/scene-reference，边界经 deps 回调解耦。

合入 `feature/camera-derive` 后真机（真实 Agnes，项目 1783192733645，空镜 #3）：
- 响应体带 `styleContractVersion: 0`（P0 前无此字段）；shot JSON 写入 `gen_style_contract_version: 0`；
- 无 `style_context_unavailable` 日志 → resolver 在位且返回有效 context；
- 图像 1024×1024 = 未锁契约（version 0）派生默认，尺寸继承路径已执行（锁定契约的 640×384 注入由 Codex 隔离接线自验以 version 7 证过，本项目不锁真实契约以免污染用户数据）；
- lint PASS，merge 后模块测试不回归。

结论：P0 合入真机 PASS。P1（referenceImages/IPAdapter）前置 A/B 未做，属下一阶段。

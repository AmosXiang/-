...
> **前置**：WP-Animatic 已合入主线（结果预览复用其 AnimaticPlayer）。
> 基线：WP-Animatic 合入后的 `feature/camera-derive` HEAD（CC 分发时填写：`________`）。
...
- **M1 只声明 agnes 一个真实 provider**。数值必须从现有事实推导并在证据文件里写明出处：`docs/video-providers/agnes-video-v2.md` + server.ts 既有创建参数（seed/num_frames/frame_rate/normalized_size）——durations 由 num_frames/frame_rate 组合枚举，aspectRatios 由 normalized_size 实际支持值推导。**禁止臆造能力**（方案 §3.1：Veo `referenceImages`≠首帧即前车之鉴）；核对不到的能力一律声明 false/空。
...
type VideoLabDeps = {
  ...
  submitVideoTask: (input: {
    ...
    seed: number; numFrames: number; frameRate: number; normalizedSize?: string;
...
校验顺序（全部 422 + code，除注明外）：
   ...
   4. **画幅继承（方案 §五A）**：项目画幅 = 风格契约 width:height 约分（契约取自 style-contract 模块导出的 `resolveEffectiveStyleContract(readDb, projectId)`，只读引用，wrap 其 PROJECT_NOT_FOUND）。契约画幅 ∈ capability.aspectRatios → 直接采用；不支持 → 若无 `aspectDecision` 返 409 `ASPECT_UNSUPPORTED` + `{ projectAspect, supportedAspectRatios }`（前端据此弹三选层）；有 `aspectDecision` 且其 aspectRatio 受支持 → 采用并记入快照（**不允许服务端擅自裁切**）。
...
- 画幅行：显示"项目画幅 X:Y（继承风格契约）"；POST 返回 409 `ASPECT_UNSUPPORTED` 时弹三选层（更换模型/裁切适配/留边适配，后两者带 aspectDecision 重发），文案照方案 §五A；
...
- 证据：`docs/ui-redesign/tasks/evidence/video-lab-m1-acceptance.md`（**必须含 agnes capability 每项数值的出处清单**）。
